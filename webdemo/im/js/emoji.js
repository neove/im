...
function buildEmoji(text) {
	...
	for (var j = 0, len = matches.length; j < len; ++j) {
		if(emoji[matches[j]]){
			text = text.replace(matches[j], '<img class="emoji" src="http://10.189.1.99:8888/webdemo/im/images/emoji/' + emoji[matches[j]].file + '" />');
		}		
	}
	return text;
}
...
CEmojiEngine.prototype.__renderChangeCol = function(){
	for(var i in this._emojiList){
		...
		for(var key in emojiList){
			...
			span.id = 'chn-emoji-'+i;	
			var img = new Image();
			img.src = "http://10.189.1.99:8888/webdemo/im/images/"+emojiList[key]['file'];
	...
	for(var i in this._pinupList){
		...
		span.id = 'chn-pinup-'+i;
		var img = new Image();
		img.src = "http://10.189.1.99:8888/webdemo/im/images/"+pinupList[0];
		span.appendChild(img);
		this._changeColumn.appendChild(span);		
	}
}
...
CEmojiEngine.prototype.__renderPictureCol= function(){
	...
	if(isEmoji){		//表情包
		...
		for(var key in emojiList){
			...
			var img = new Image();
			img.src = 'http://10.189.1.99:8888/webdemo/im/images/emoji/'+emojiList[key]['file'];
			span.appendChild(img);
		...
		for(var key = 0;key<pinupList.length;key++){
			...
			span.style.height= 'auto';
			var img = new Image();
			img.src = 'http://10.189.1.99:8888/webdemo/im/images/' +subtype+ '/'+pinupList[key];
			span.appendChild(img);
			this._pictureColumn.appendChild(span);		
		}		
	}
