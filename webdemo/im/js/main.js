...
var yunXin = {
    ...
    initNode: function () {
        ...
        //下面实验跳过好友验证直接发起会话
        this.$userBox = $(".itemstaff")
        this.$imAdd = $("#im_add");//添加好友
        //点击在线头像发起会话
    ...
    addEvent: function () {
        ...
        this.$imAdd.bind("keydown", function (e) {
            if (e.keyCode === 13) {
                var account = $.trim($(this).val().toLowerCase());
                that.showAddFriend();
                $('#addFriendBox .j-account').val(account);
                $(this).val('');
            }
        })
        ...
        //搜索框获取焦点的用户交互
        this.$imAdd.focus(function () {
            that.$imAdd.addClass('im_active')
        }).blur(function () {
            that.$imAdd.removeClass('im_active')
        });
        this.$messageText.focus(function () {
            that.$messageText.addClass('im_active')
        }).blur(function () {
            that.$messageText.removeClass('im_active')
        });
        ...
        $('#addFriendBox .btns .j-chat').bind('click', function () {
            $('#im-wrapper').css({ "right": "0" })//划出会话框
        })
        //////////////////////////////////////////////////////////////////////////////////////////下面实验跳过好友验证发起会话
        setInterval(function () {
            if ($('.itemstaff').length !== 0) {
                if ($('.chat_icon').length == 0) {
                    $('.main_is .tagarea').each(function () {
                        var $img = $('<img src="http://10.189.1.99:8888/webdemo/im/images/im_add.png"  class="imadd_icon" >');
                        $img.appendTo($(this));
                        $img.bind('click', function () {
                            that.showAddFriend();
                            $('#addFriendBox .j-account').val("beisen021");
                        })
                    })
                    $('.itemstaff .main_is p').each(function () {
                        var $img = $('<img src="http://10.189.1.99:8888/webdemo/im/images/im_chat.png"  class="chat_icon" >');
                        $img.appendTo($(this));
                        $img.bind('click', function () {
                            var user_name = $(this).parent().parent().find('.name_is').attr('original-title');
                            $('#im-wrapper').css({ "right": "0" })//划出会话框
                            switch (user_name) {
                                case "李国明":
                                    that.openChatBox("beisen002", "p2p");
                                    break;
                                case "青岛李贺":
                                    that.openChatBox("beisen003", "p2p");
                                    break;
                                case "徐海彬":
                                    that.openChatBox("beisen000", "p2p");
                                    break;
                                case "张倩":
                                    that.openChatBox("beisen001", "p2p");
                                    break;
                                case "尹吉凯":
                                    that.openChatBox("beisen004", "p2p");
                                    break;
                                case "孔常柱":
                                    that.openChatBox("beisen005", "p2p");
                                    break;
                                default:
                                    that.openChatBox("beisen010", "p2p");
                                    $('#j-nickName').html(user_name);
                                    //$('.j-session .active .panel_avatar img').attr('src')
                                    $('.j-session .active .panel_nick').html(user_name)
                            }
                        })
                    });
                }
    ...
    //左栏上方自己的信息
    //在这里修改下，同步的是tita的账户信息
    showMe: function () {
        // var user = this.cache.getUserById(userUID);
        var tita_user = $('.user-info-region .user-name').html();
        this.$userName.text(tita_user);
        var tita_pic = $('.user-photo img').attr('src');
        this.$userPic.attr('src', tita_pic);
        ...
        switch (type) {
            case "session":
                ...
                if (scene === "p2p") {
                    ...
                    if (msg.target === userUID) {
                        info.nick = "我的手机";
                        info.avatar = "http://10.189.1.99:8888/webdemo/im/images/myPhone.png";
                    } else {
                    ...
                    if (teamInfo) {
                        ...
                        if (teamInfo.avatar) {
                            ...
                            info.avatar = "http://10.189.1.99:8888/webdemo/im/images/" + teamInfo.type + ".png";
                        }
                    } else {
                        info.nick = msg.target;
                        info.avatar = "http://10.189.1.99:8888/webdemo/im/images/normal.png";
                    }
                }
                break;
            case "friend":
            ...
            case "team":
                ...
                if (data.avatar) {
                    ...
                    info.avatar = info.type === "normal" ? "http://10.189.1.99:8888/webdemo/im/images/normal.png" : "http://10.189.1.99:8888/webdemo/im/images/advanced.png";
                }
                info.crtSession = this.crtSession;
                break;
        }
    ...
    showUnread: function () {
        ...
        var $node = $(".u-unread");//这里做了了个改动  去掉了m-unread 直接找u-unread
    ...
    openChatBox: function (account, scene) {
        ...
        if (scene == "p2p") {
            ...
            if (info) {
                ...
                this.$chatTitle.find('img').attr('src', "http://10.189.1.99:8888/webdemo/im/images/normal.png");
