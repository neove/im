...
fs.readFile(path.join(__dirname, 'userdata.js'), function (err, bytesRead) {
    if (err) throw err;
    user = JSON.parse(bytesRead);
});

//存储user信息
function saveUser(user) {
	fs.writeFile(path.join(__dirname, 'userdata.js'), JSON.stringify(user), function (err) {
        if (err) throw err;
...
//检测用户是否已经注册im
function checkUser(accid) {
	for (var i = 0; i < user.length; i++) {
		if (user[i].info.accid === accid) return user[i];
	}
	return 0;
}
//将新注册的用户拉入一个群，以便测试
...
http.createServer(function (req, res) {
	...
	res.setHeader("Access-Control-Allow-Origin", "*")//解决跨域请求的问题
	if (req.url == '/') {
		var data = '';
		var imdata;
		...
		req.on('data', function (chunk) {
			imdata = querystring.parse(data += chunk);//转成对象的格式，然后发给起注册请求
		})
		req.on('end', function () {
			console.log(imdata)
			myIm.createUserId(imdata, function (err, data) {//调用注册接口
				console.log(data);
				if (data.code === 200) {
					user.push(data);
					saveUser(user);
